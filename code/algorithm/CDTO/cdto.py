...
class Simulator_greedy:
        ...
        for q in range(127):
            for j  in range(len(self.task_iner_priority)):
                ...
                if layer.name == 'output' or layer.name == 'data' :
                    ...
                    layer_end_time ,device= self.update_time1(j, layer)
                    layer.device_id = device
    ...
    def update_time(self, i, device, layer):
        ...
        for parent in layer.inbounds:
            if parent.device_id != device:
                ...
                finish_trans_tensor = next((m for m in self.transfer_device[i]
                ...
                if finish_trans_tensor == None:
                    ...
                    self.transfer_device[i].append([parent, parent.device_id, device,
                                                             trans_start_time,
                                                             trans_end_time])
    ...
    def update_time1(self, i,  layer):
        min = 1000
        last_device = 0
        last_layer_start_time = 0
        # 在多个设备中选出完成时间最少的
        for device in range(len(self.device_graph.devices)):
        ...
        layer_end_time = self.update_time(i, last_device, layer)
